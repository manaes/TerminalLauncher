...
| 단위 회귀 | 별도 `swift Tests/run_unit_tests.swift` 가 `passed: 112 / failed: 0` (셸 escape · 빌드 명령 합성 · EntryStore 정합성 · 토큰 정규식 · 10MB 경계 · `.pathdock` 헤더 · SSH 셸 합성 · VNC URL 합성 · SSH config 파서 + extraOptions · `-oKey=Value` 합성) |
